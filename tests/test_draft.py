...
``call_llm`` is replaced with a fake that drives the node's ``on_token`` callback
exactly as the real streaming client does, so the bus contract is exercised
without a socket.
"""
...
async def test_tokens_are_published_and_the_draft_is_their_join(
    ...
    monkeypatch.setattr(draft_prose_module, "call_llm", _streaming_call_llm())
...
async def test_beat_start_is_published_once_before_the_first_token(
    ...
    assert beat_start["word_target"] == 600
...
async def test_the_prompt_carries_the_beat_and_its_pad_constraint(
    ...
    seen: list = []
    monkeypatch.setattr(
        draft_prose_module, "call_llm", _streaming_call_llm(seen_messages=seen)
    )
...
async def test_drafting_without_an_assembled_context_is_a_hard_failure(
    ...
    monkeypatch.setattr(draft_prose_module, "call_llm", unreachable)
...
async def test_an_empty_stream_is_a_hard_failure(configured, monkeypatch):
    monkeypatch.setattr(draft_prose_module, "call_llm", _streaming_call_llm(tokens=[]))
