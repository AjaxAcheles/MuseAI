...
An interactive debugger for the five-level planner cascade — the bounded "model proposes, harness disposes" loop. An **LLM mode** dropdown at the top of the page selects the decider behind the seam. In the default **Mocked LLM** mode the model is replaced by a **scripted decider** — an editable JSON list of `PlannerAction`s the loop consumes one per turn — so no live model or network is ever contacted (`call_llm` is never reached). In **Real LLM** mode the mock is bypassed: the node builds its production M04-backed decider and every deliberation turn is a live structured-output call to the configured planner endpoint (secrets from `.env`). In both modes the beat level's PAD translation runs with `adapt_fn=None` (the LLM-free static rung — the adapter's prompt template is not authored yet), and everything else is production code end-to-end: constraint compilation, the deliberation loop, validator gating, the fallback ladder, tool dispatch through the permission matrix, and persistence through the 07.00 helpers.

#### Run controls

| Control | Type | What it does |
|---------|------|-------------|
| `LLM mode` | Select (top of page) | **`Mocked LLM` (default)** — the scripted decider stands in for the model, zero network. **`Real LLM`** — bypasses the mock: the node builds its production decider (`make_planner_decider` → `call_llm_structured`) against the `config.yaml` endpoints, loading `*_API_KEY` secrets (and optional `*_BASE_URL` overrides) from the repo `.env`; the scripted-decider controls are disabled. The choice is mirrored into a plain session key so it survives page switches inside `run_all.py`. |
